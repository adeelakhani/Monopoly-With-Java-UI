...
public class Monopoly extends JFrame implements ActionListener {
	...
	public Monopoly() {
		...
		JPanel DeedCard = new JPanel(); // add a blank image over the spot of where the title deed cards are supposed to
										// be, because players start on GO, so its going to be blank
		...
		constraints.gridx = 9;
		constraints.gridy = 12;
		constraints.gridwidth = 3; // since any images placed determine the length or the width of the column or
									// row respectively, so I had to add this to say that this panel will take up 3
									// columns
		...
		constraints.gridx = 3;
		constraints.gridy = 3; // added the properties list for both players above the chest and community
								// cheest cards
		...
		while (p1Money >= 0 && p2Money >= 0) // loop will run as long as both players arent bankrupt
		{
			...
			if (turn == true) // turn = true means its player 1'a turn
			{
				...
				if (p1Jail == true) // first check if they are in jail
				{
											...
											// buttons
					{
						...
						player2Roll = true;
						if (turn == false) // once they click the wait turn button, there turn is over so once that
											// happens we break out of loop
						{
							break;
						}
					}
				...
				} else // if player 1 is not in jail
				{
					...
					if (p1Pos == 30) // if they land on the GO-TO-JAIl
					{
						...
						commentary.setText(
								word + "\n" + p1Name + ": You have been sent to Jail, your turn automatically ends"); // updating
																														// the
																														// commentary
																														// field
						...
						if (p1jailCard == false) // if they dont have a get out of jail free card
						{
							...
							p2flag = true; // rolling for pplayer 2 will only work if player 2 flag is false, this avoid
											// pl1 and pl2 rolling at the same time
						} else // if they do, they are in just visiting
						{
							word = commentary.getText();
							commentary.setText(word + "\n" + p1Name
									+ ": You would have been sent to Jail, but you had a Get-Out-of-Jail free card, so you are just visiting");
					...
					posButtonsRed[p1Pos].setBackground(Color.red); // whatever the pos is from above, color in the next
																	// position

					if (p1Pos == 7 || p1Pos == 22 || p1Pos == 36) // IF they land on a chance position on the
																	// board.Chance Start
					{
						...
						} else if (randChoice == 5) // Chairman
						{
							...
							word = commentary.getText();
							commentary.setText(word + "\nChance: " + p1Name
									+ ": Elected chairman of the board, pay $50 to other player");
							p1Money -= 50;
						...
						} else if (randChoice == 7) // Go tO Jail
						{
							...
							if (p1jailCard == true) // if they have a get out of jail free card
							{
								...
								word = commentary.getText();
								commentary.setText(word + "\n" + p1Name
										+ ": You have been sent to Jail, your turn automatically ends"); // if they
																											// dont,
																											// they arte
																											// in jail,
																											// similar
						...
						} else if (randChoice == 9) // Out of Jail Free card, will be used right away
						{
							...
							word = commentary.getText();
							commentary.setText(word + "\nChance: " + p1Name
									+ ": Get out of Jail Free Card(will be used at first instance of jail");

						}

						ChanceCardA = new JPanel(); // creating a panel to add the actual chance card
						posChance = new JLabel(Chance[randChoice]); // creating a label with the images file of all the
																	// chance as the randChoice as index to ensure card
					...
					{
						int randChoice = (int) (Math.random() * 13) + 1; // same as the chance just different actions,
																			// jail and out of jail free cards are the
																			// same
						randChoice -= 1;
						...
						} else if (randChoice == 1) // Beauty Contest
						{
							p1Money += 10; // A
							word = commentary.getText();
							commentary.setText(word + "\nCommunity Chest:" + p1Name
									+ ": You placed second place in a beauty contest, collect $10");
						...
						} else if (randChoice == 6) // Go to Jail Community Chest
						{
							...
							if (p1jailCard == true) {
								...
								word = commentary.getText();
								commentary.setText(word + "\n" + p1Name
										+ ": You have been sent to Jail, your turn automatically ends");
								player2Roll = true;
								p1Jail = true;
								turn = false;
								p2flag = true;
					...
					if (p1Pos != 5 && p1Pos != 15 && p1Pos != 25 && p1Pos != 35 && p1Pos != 12 && p1Pos != 28) // Checking
																												// if
																												// player
																												// lands
																												// on a
																												// property
					{
						if (buyableProperties[p1Pos] == false) // checking if its owned or not
						{
							...
							String p1Check[] = new String[player2Assets.size()]; // checking player 2's vector to see if
							...
							if (found >= 0) {
								...
								p1Money -= rentData[p1Pos][propertyUpgrades[p1Pos]]; // if they do, using the 2d array,
																						// subtract the rent, row is the
																						// position p1 is on, and column
																						// is the rent
								p2Money += rentData[p1Pos][propertyUpgrades[p1Pos]];
								word = commentary.getText();
								commentary.setText(word + "\n " + p1Name + " pays rent to " + p2Name + " at "
										+ p1Check[found] + " for " + (rentData[p1Pos][propertyUpgrades[p1Pos]]));
							}
						}
					}
					if (p1Pos == 5 || p1Pos == 15 || p1Pos == 25 || p1Pos == 35) // if p1 lands on a railroad owned by
																					// p2
					{
						...
						if (found >= 0) {
							int total = 25;
							for (int count = 1; count <= p1numRR; count++) // depending on the number of railraod we
																			// double the rent each time
							{
								total = total * 2;
							}
							p1Money -= total;
					...
					while (turn == true) // as long as its player 1's turn
					{
						...
						if (turn == false) // once they end turn, chance and community chest are returned back to the
											// back side of the card and the title deed card is blank before the next
											// players turn
						{
							DeedCard = new JPanel();
			...
			else // All of player 1's options are mirrored in player 2's rolling
			{
				if (p2Jail == true) {
					...
					if (p2Pos != 5 && p2Pos != 15 && p2Pos != 25 && p2Pos != 35 && p2Pos != 12 && p2Pos != 28) // 12,28
																												// utilites,
																												// others
																												// railroad
					{
						if (buyableProperties[p2Pos] == false) {
							...
							if (found >= 0) {
								...
								word = commentary.getText();
								commentary.setText(word + "\n " + p2Name + " pays rent to " + p1Name + " at "
										+ p2Check[found] + " for " + (rentData[p2Pos][propertyUpgrades[p2Pos]]));
							}
						}
					}
	...
	public void actionPerformed(ActionEvent evt) {
		...
		{
			if (player1Roll == true && turn == true && p1Jail == false && p1flag == false) // p1 must be able to roll,
																							// it must be there turn,
																							// they cannot be in jail
			{
			...
			} else if (player2Roll == true && turn == false && p2Jail == false && p2flag == false) // same thing but for
																									// player 2
			{
				dice = diceRoll();
				word = commentary.getText();
		...
		{
			if (turn == true && player1Roll == false && p1Jail == false) // if they cannot roll, its their turn, and
		...
		if (evt.getActionCommand().equals("UPGRADE")) {
			if (turn == true && p1Jail == false) // not in jail, their turn
			{
				...
				{
					if (buyableProperties[p1Pos] == false && propertyUpgrades[p1Pos] < 6) // property is owned and also
	...
	public void UpgradingP1() {
		String word = "";
		if (p1Pos >= 0 && p1Pos <= 10) // depending on the side of the board, the money of the price to upgrade
										// increases
		{
			if (50 > p2Money) // if they don't have enough money, give the error message
			{
				...
				p1Money -= 50;
				if (p1Pos == 1) // If you buy the upgrade, depending on the level of the property at that
								// position, we colour in the corresponding house
				{
					mediterraneanhouses[propertyUpgrades[p1Pos] - 1].setBackground(Color.red); // red because player one
																								// is red
				}
				if (p1Pos == 3) {
	...
	public void Buying() //
	{
		if (turn == true && p1Rights == true) // if the player can buy and its their turn
		{
			...
			} else // if they do, do the transaction
			{
				...
				if (p1Pos == 5 || p1Pos == 15 || p1Pos == 25 || p1Pos == 35) // if its a railraod increase the number of
																				// railroads owned
				{
					p1numRR++;
				}
...
class LandingPage25 extends JFrame implements ActionListener // landing page
{
	...
	public void doMyButtonAction() // once start is clicked, add the names from the textfield into the array and
									// send the array back to the main code
	{
...
class EndScreen extends JFrame // end screen
{
	...
	void EndingScreen(int p1Money, int p2Money, String p1Name, String p2Name) {
		...
		try {
			...
			scores = new JLabel("Overall Wins till now:        Player 1:   " + score1 + "     Player 2:   " + score2); // gets
																														// scores
																														// from
																														// file
																														// and
